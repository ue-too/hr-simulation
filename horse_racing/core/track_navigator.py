...
Adapted from v1 with turn_direction added to TrackFrame for L/R grip differentiation.
"""
...
from horse_racing.core.track import compute_segment_length, compute_total_length
...
class TrackNavigator:
    ...
    def __init__(self, segments: list[TrackSegment]) -> None:
        ...
        self.entry_radius: float = float("inf")
    ...
    def _straight_frame(self, seg: StraightSegment, position: np.ndarray) -> TrackFrame:
        ...
        normal = self._outward_normals[self.segment_index]
        return TrackFrame(
    ...
    def _curve_frame(self, seg: CurveSegment, position: np.ndarray) -> TrackFrame:
        ...
        inv_dist = 1.0 / dist
        nx, ny = tpx * inv_dist, tpy * inv_dist
        ...
        # Cross product of tangential x (center - position):
        # positive = center is to the left = left turn
        # negative = center is to the right = right turn
        to_center_x = seg.center[0] - float(position[0])
        to_center_y = seg.center[1] - float(position[1])
        cross = float(tangential[0]) * to_center_y - float(tangential[1]) * to_center_x
        turn_direction = 1.0 if cross > 0 else -1.0  # +1 = left, -1 = right
